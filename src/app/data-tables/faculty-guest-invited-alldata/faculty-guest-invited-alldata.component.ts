...
@Component({
  selector: 'app-faculty-guest-invited-alldata',
  templateUrl: './faculty-guest-invited-alldata.component.html',
  styleUrls: ['./faculty-guest-invited-alldata.component.scss']
})
export class FacultyGuestInvitedAlldataComponent implements AfterViewInit {
  ...
  fileName = 'Faculty List Form A (Guest Lecturer | Invited Foreigner Lecturers).xlsx';
    ...
    getDocs(
      ...
      this.faculty=[...res.docs.map((doc:any)=>{
        return {...doc.data(),id:doc.id}
          ...
          this.faculty = this.faculty.filter(
          (item: { sub_type: string }) => item.sub_type == 'invited foreign lecturer'
        );
  ...
  exportexcel(): void {
    ...
    let element = document.getElementById('table-invited-alldata');
  ...
  openModal(){
    let modal = this.modal.open(ViewFacultyComponent,{
      ...
      data:{
        type:'teaching-staff',
        sub_type:'invited foreign lecturer'
      }
